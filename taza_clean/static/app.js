...
async function autoAuthenticate() {
  try {
    // 1. Consumer Demo Login
    const consumerRes = await fetch(`${API_BASE}/auth/login/json`, {
      method: "POST",
    ...
    if (consumerRes.ok) {
      ...
      authTokens.consumer = data.access_token;
      currentUser.id = data.user_id;
      currentUser.name = data.full_name;
    }

    // 2. Farmer Demo Login
    const farmerRes = await fetch(`${API_BASE}/auth/login/json`, {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({
    ...
    if (farmerRes.ok) {
      ...
      authTokens.farmer = data.access_token;
    }
  } catch (err) {
    console.warn("Backend auth offline or initializing. Local fallback ready.", err);
  }
}
